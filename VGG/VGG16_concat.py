...
class VGG16_concat(object):
    ...
    def __init__(self, scope_name, image_width = 224, image_height = 224, image_depth = 3, num_labels = 17, bn_option=0, phase=0):
        ...
        with tf.variable_scope(self.scope_name):
            ...
            self.w14 = tf.Variable(name="w14", initial_value=tf.truncated_normal([1530368 , 4096], stddev=0.1))
            ...
            self.layer1_relu  = self.conv_layer(input=self.input, weight=self.w1, bias=self.b1, bn_options=self.bn_option)
            ...
            self.flat1        = tf.contrib.layers.flatten(self.layer2_pool)
            self.flat2        = tf.contrib.layers.flatten(self.layer4_pool)
            self.flat3        = tf.contrib.layers.flatten(self.layer7_pool)
            self.flat4        = tf.contrib.layers.flatten(self.layer10_pool)
            self.flat5        = tf.contrib.layers.flatten(self.layer13_pool)
            self.flat_layer   = tf.concat(values=[self.flat1, self.flat2, self.flat3, self.flat4, self.flat5], axis=1)
            print("SHAPE OF FLAT LAYER")
            print(tf.shape(self.flat_layer))

            self.layer14_relu = self.feed_forward_layer(input=self.flat_layer, weight=self.w14, bias=self.b14, bn_options=bn_option)
            
            self.layer15_relu = self.feed_forward_layer(input=self.layer14_relu, weight=self.w15, bias=self.b15, bn_options=bn_option)
    ...
    def training_step(self, learning_rate=0.05, learning_rate_type="STATIC", optimizer_type="SGD", bn_option=0, **kwargs):
        '''
